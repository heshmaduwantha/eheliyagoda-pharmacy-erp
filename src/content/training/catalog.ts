...
export const trainingCategories: TrainingCategory[] = [
  ...
  { key: "expiry", titleSi: "Expiry සහ Expired Stock", titleEn: "Expiry and expired stock", descriptionSi: "Near-expiry alerts සහ expired stock removal workflow." },
...
export const scenarioLessons: TrainingLesson[] = [
  ...
  {
    key: "scenario.expiry", slug: "expiry", kind: "scenario", category: "expiry", titleSi: "Near-expiry සහ expired stock", titleEn: "Near-expiry and expired stock", summarySi: "Expiry alerts කියවීම සහ expired stock manual removal workflow එක තේරුම් ගැනීම.", difficulty: "intermediate", estimatedMinutes: 12,
    businessContext: "Expired medicine sell නොකිරීම නීතිමය සහ patient-safety අවශ්‍යතාවයකි.", prerequisites: ["Stock read permission", "Expired Stock removal permission"], requiredPermissions: ["inventory.stock.read", "inventory.stock.writeoff"], relatedRoute: "/stock/batches", relatedRouteLabel: "Batch register විවෘත කරන්න",
    steps: [
      ...
      { title: "Expired Stock remove කරන්න", page: "Stock → Batch register", action: "Expired batch එක සඳහා Remove click කර confirmation එක දෙන්න.", result: "Remaining quantity එක WRITE_OFF movement එකක් ලෙස record වී batch එක DEPLETED වේ." },
    ], dataImpacts: ["Expiry alerts read-only.", "Expired Stock remove කළ විට remaining quantity එක WRITE_OFF movement එකක් ලෙස record වේ.", "Expired stock delete කිරීම කිසිවිටෙක නිර්දේශ නොකරයි."], notices: [{ tone: "warning", title: "Manual action අවශ්‍යයි", body: "Expired Stock removal automatic නොවේ. Physical stock වෙන් කර authorised user කෙනෙක් විසින් batch register එකෙන් Remove කරන්න." }], checklist: ["Expired stock physical shelf එකෙන් වෙන් කළාද?", "Expiry alert evidence සටහන් කළාද?", "Expired Stock removal confirm කළාද?"], relatedLessons: ["module.expiry-alerts", "scenario.daily-operations"], diagram: [{ label: "ACTIVE", note: "Sellable if unexpired" }, { label: "Near expiry", note: "Alert only" }, { label: "Expired Stock", note: "Manual removal creates WRITE_OFF movement" }, { label: "DEPLETED", note: "Quantity becomes 0" }],
  },
  {
...
export const glossary = [
  ...
  ["Expired Stock", "Expired stock එක audited movement එකක් සමඟ ඉවත් කිරීම.", "Batch register එකේ Remove action භාවිතා කරන්න."],
