...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const users = await listAdminUsers({ search: params.search, status });
    ...
    <div className="grid gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">Users</h1>
          <p className="mt-2 text-slate-500">Create staff accounts, activate/deactivate users, and assign roles.</p>
        </div>
        <Link className="inline-flex items-center gap-2 rounded-xl bg-teal-700 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-teal-700/20" href="/admin/users/new">
      ...
      <form className="grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_220px_auto]">
        <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3">
          ...
          <input className="min-w-0 flex-1 bg-transparent py-2.5 text-sm outline-none" defaultValue={params.search} name="search" placeholder="Search name, username, or role…" />
        </label>
          ...
          <table className="w-full min-w-[1100px] border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <tr className="align-top hover:bg-teal-50/30" key={user.id}>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700" href={`/admin/users/${user.id}`}>
                          Edit
                        </Link>
                        <form action={toggleUserActiveFormAction}>
                          <input name="userId" type="hidden" value={user.id} />
                          <input name="nextActive" type="hidden" value={user.isActive ? "false" : "true"} />
                          <button className={`rounded-lg px-3 py-2 text-sm font-semibold ${user.isActive ? "border border-rose-200 bg-rose-50 text-rose-700" : "border border-emerald-200 bg-emerald-50 text-emerald-700"}`} type="submit">
