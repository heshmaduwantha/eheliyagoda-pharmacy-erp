      ...
      <div className="flex min-h-11 flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 focus-within:border-teal-400">
        {selectedUnits.map((unit) => (
          <span className="inline-flex items-center gap-1 rounded-full bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-700" key={unit}>
            {unit}
            <button aria-label={`Remove ${unit}`} className="rounded-full p-0.5 hover:bg-teal-100" onClick={() => toggleUnit(unit)} type="button">
              <X className="size-3" />
            </button>
          </span>
        ))}
        <input
          ...
          value={query}
        />
        {selectedUnits.length > 0 ? <button className="text-xs font-semibold text-slate-500 hover:text-slate-800" onClick={() => onChange([])} type="button">Clear all</button> : null}
        <button aria-expanded={isOpen} aria-label="Toggle unit options" className="rounded p-1 text-slate-400 hover:bg-slate-100" onClick={() => setIsOpen((open) => !open)} type="button">
          <ChevronDown className="size-4" />
        </button>
      </div>
      {isOpen ? (
        <div className="absolute z-20 mt-2 max-h-56 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg" role="listbox" aria-label="Sold in units">
          {visibleOptions.length > 0 ? visibleOptions.map((unit) => {
            const selected = selectedUnits.includes(unit);
            return (
              <button aria-selected={selected} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm ${selected ? "bg-teal-50 font-semibold text-teal-700" : "text-slate-700 hover:bg-slate-50"}`} key={unit} onClick={() => toggleUnit(unit)} role="option" type="button">
                {unit}
                {selected ? <Check className="size-4" /> : null}
              </button>
            );
          }) : <p className="px-3 py-2 text-sm text-slate-400">No matching units.</p>}
        </div>
      ) : null}
    </div>
  );
    ...
    <form action={formAction} className="grid gap-5" ref={formRef}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="grid gap-1">
          <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Sold in</span>
          <SoldInMultiSelect onChange={updateSelectedUnits} selectedUnits={selectedUnits} />
          {state.status === "error" && state.fieldErrors?.units ? <span className="text-xs font-semibold text-red-600">{state.fieldErrors.units}</span> : <span className="text-xs text-slate-400">Select every unit customers can buy. Add the exact conversion for each selection below.</span>}
        </div>
      ...
      {selectedUnits.length > 0 ? <section className="rounded-xl border border-slate-200 bg-slate-50 p-4">
        <div>
          <p className="text-sm font-bold text-slate-700">Unit conversions &amp; barcodes</p>
          <p className="mt-1 text-xs text-slate-500">A conversion is required for each selected unit. For example, 1 Box = 10 {baseUnitName || "base units"}.</p>
        </div>
        <div className="mt-3 grid gap-2">
          {selectedUnits.map((unit, index) => {
            const details = unitDetails[unit] ?? emptyUnitDetails();
            return <div className="grid items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 sm:grid-cols-[1fr_1fr_1.4fr_auto]" key={unit}>
              <strong className="text-sm text-slate-800">{unit}{index === 0 ? <span className="ml-2 text-xs font-medium text-teal-700">Default sale unit</span> : null}</strong>
              ...
              <label className="flex items-center gap-1.5 whitespace-nowrap text-xs font-semibold text-slate-600"><input checked={details.isPurchaseDefault} onChange={(event) => updateUnitDetails(unit, { isPurchaseDefault: event.target.checked })} type="checkbox" /> Default purchase</label>
            </div>;
          })}
        </div>
      </section> : null}
      ...
      <details className="rounded-xl border border-slate-200 bg-slate-50">
        <summary className="flex cursor-pointer items-center gap-2 px-4 py-3 text-sm font-semibold text-slate-600 marker:content-none"><ChevronDown className="size-4 text-slate-400" />More options<span className="ml-1 text-xs font-normal text-slate-400">(generic name, category, prescription…)</span></summary>
        <div className="grid gap-4 border-t border-slate-200 px-4 pb-4 pt-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><label className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-sm font-semibold text-amber-800"><input checked={isControlled} name="isControlled" onChange={(event) => setIsControlled(event.target.checked)} type="checkbox" className="mt-0.5" /><span>This is a controlled drug (requires patient + prescriber details at every sale)</span></label></div>
        </div>
      </details>
