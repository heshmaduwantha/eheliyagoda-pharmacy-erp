          ...
          <table className="w-full min-w-[1000px] border-collapse text-sm">
            <thead>
              <tr className="bg-brand-pale text-left text-xs font-extrabold uppercase tracking-wider text-brand-hover border-b border-brand-default/15">
                ...
                <th className="px-3 py-3 text-status-danger-text font-extrabold">Unit cost *</th>
                <th className="px-3 py-3 text-status-danger-text font-extrabold">Unit price *</th>
              ...
              {lines.map((line, index) => {
                ...
                return (
                  <tr className="border-t border-neutral-border" key={index}>
                    ...
                    <td className={`${cell} min-w-[95px]`}>
                        <input aria-label="Unit purchase cost" className={`${inputClass} ${!line.costPrice || Number(line.costPrice) <= 0 ? "border-status-danger-text focus:ring-status-danger-text" : ""}`} min="0.01" onChange={(e) => updateLine(index, { costPrice: e.target.value })} placeholder="Unit cost" required step="0.01" type="number" value={line.costPrice} />
                    </td>
                    <td className={`${cell} min-w-[95px]`}>
                        <input aria-label="Unit selling price" className={`${inputClass} ${!line.sellingPrice || Number(line.sellingPrice) <= 0 ? "border-status-danger-text focus:ring-status-danger-text" : ""}`} min="0.01" onChange={(e) => updateLine(index, { sellingPrice: e.target.value })} placeholder="Unit price" required step="0.01" type="number" value={line.sellingPrice} />
                    </td>
