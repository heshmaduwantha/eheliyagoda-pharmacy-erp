...
export function InventoryFilters({ 
  ...
  return (
    <form action={action} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row flex-wrap">
      ...
      {directionOptions.length > 0 ? <label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm text-slate-500 shrink-0">
        <Filter className="size-4 shrink-0" />
        <select className="bg-transparent py-2.5 font-semibold text-slate-700 outline-none" defaultValue={direction} name="direction">
          <option value="ALL">All directions</option>
          {directionOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
        </select>
      </label> : null}

      {showAvailability ? <label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm text-slate-500 shrink-0">
        <Filter className="size-4 shrink-0" />
      ...
      {showTimeframe ? <label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm text-slate-500 shrink-0">
        <Filter className="size-4 shrink-0" />
