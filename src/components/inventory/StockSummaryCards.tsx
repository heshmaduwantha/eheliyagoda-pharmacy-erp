...
export function StockSummaryCards({ summary }: { summary: StockSummary }) {
  const cards = [
    { label: "Active products", value: summary.totalActiveProducts, note: "Mock catalog coverage", icon: Boxes, tone: "teal" },
    { label: "Low stock", value: summary.lowStockCount, note: "Below preview threshold", icon: AlertTriangle, tone: "amber" },
    { label: "Near expiry", value: summary.nearExpiryCount, note: "Within 90 days", icon: CalendarClock, tone: "blue" },
    { label: "Expired / quarantined", value: summary.expiredOrQuarantinedCount, note: "Read-only alert count", icon: ShieldAlert, tone: "red" },
  ] as const;
