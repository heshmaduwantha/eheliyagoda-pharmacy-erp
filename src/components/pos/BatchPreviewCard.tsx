...
export function BatchPreviewCard({ batch }: { batch?: PosBatchPreview }) {
  ...
  return <div className={`grid grid-cols-2 gap-2 rounded-xl border p-2.5 text-[11px] ${batch.canFulfil ? "border-teal-100 bg-teal-50/60" : "border-amber-100 bg-amber-50"}`}><span className="flex items-center gap-1.5 text-slate-600"><PackageCheck className="size-3.5 text-teal-600" />{candidate.batchNumber ?? "No batch number"}</span><span className="text-right font-semibold text-slate-700">MRP {candidate.mrp ? formatLkr(Number(candidate.mrp)) : "—"}</span><span className="flex items-center gap-1.5 text-slate-500"><CalendarClock className="size-3.5 text-amber-500" />Exp {candidate.expiryDate ?? "—"}</span><span className="text-right text-slate-500">Available {batch.totalAvailableQtyBase}</span><span className="col-span-2 text-slate-500">Advisory FEFO preview only · {batch.canFulfil ? "quantity available" : "insufficient quantity"}</span></div>;
}
