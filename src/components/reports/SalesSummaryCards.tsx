...
export function SalesSummaryCards({ summary, message }: { summary: DailySalesSummary | null; message?: string }) {
  if (!summary) return <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><ReceiptText className="mx-auto size-9 text-slate-300" /><p className="mt-3 font-bold text-slate-700">{message ?? "No completed sales yet"}</p></div>;
  const cards = [
  ...
  return <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({ label, value, icon: Icon }) => <article className="rounded-2xl border border-slate-200 bg-white p-5" key={label}><Icon className="size-5 text-teal-600" /><p className="mt-4 text-sm text-slate-500">{label}</p><p className="mt-1 text-2xl font-black text-slate-900">{value}</p></article>)}</section>;
}
